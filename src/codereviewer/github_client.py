import os
import logging
from github import Github

logger = logging.getLogger(__name__)

def post_comments_and_summary(review: dict, summary: dict) -> None:
    """
    Posts the comments and summary to the GitHub PR.
    """
    github_token = os.getenv("GITHUB_TOKEN")
    repo_name = os.getenv("GITHUB_REPOSITORY")
    pr_number = int(os.getenv("GITHUB_PULL_REQUEST_NUMBER"))
    
    if not github_token or not repo_name or not pr_number:
        logger.error("Missing required environment variables")
        return
    
    gh_client = Github(github_token)
    repo = gh_client.get_repo(repo_name)
    pr = repo.get_pull(pr_number)
    
    # post the summary comment at the top of the pr
    summary_comment = f"## Code Review Summary\n\n{summary['summary']}"
    pr.create_issue_comment(summary_comment)
    logger.info("Summary comment posted")

    # go through the review and post the comments on their respective lines
    for issue in review.get("issues", []):
        comment_body = f"**{issue['category']}**: {issue['issue']}\n\n**Impact**: {issue['impact']}\n\n**Recommendation**: {issue['recommendation']}"
        try:
            pr.create_review_comment(
                body=comment_body,
                commit_id=pr.head.sha,
                path=issue['file'],
                line=issue['line']
            )
            logger.info(f"Comment posted for {issue['file']} on line {issue['line']}")
        except Exception as e:
            logger.error(f"Error posting comment for {issue['file']} on line {issue['line']}: {e}")

    logger.info("All comments posted")